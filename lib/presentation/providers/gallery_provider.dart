// Gallery Provider - State management galeri foto
...
class GalleryState {
  ...
  const GalleryState({this.photos = const [], this.isLoading = false, this.error});

  GalleryState copyWith({List<GalleryPhoto>? photos, bool? isLoading, String? error}) => GalleryState(
...
class GalleryNotifier extends StateNotifier<GalleryState> {
  ...
  Future<GalleryResult> uploadPhoto(String imagePath, String? caption) async {
    ...
    if (result.isSuccess) await loadPhotos();
    state = state.copyWith(isLoading: false, error: result.message);
    return result;
  }
}

final galleryProvider = StateNotifierProvider<GalleryNotifier, GalleryState>((ref) => GalleryNotifier());
